...
CHAR8S UVIS25_INITIAL(void)
{
	CHAR8S status = 0;
		...
		*/
		...
		if(status !=0) 
		{
			return -1; /*set INT & INT CFG mode fail*/
		}
...
CHAR8S UVIS25_SET_MODE(CHAR8U mode_update_type)
{
	...
	CHAR8U ctrl1_update = 0;
		...
		if(mode_update_type & 0xF0)
		{
			return -2; /*mode_update_type error value*/
		}

		/*check one shot & continue read data*/
		if(mode_update_type & 0x01)  
		{
			ctrl1_update |= 0x00 ; /*set one shot mode*/	
		}

		if(mode_update_type & 0x02)
		{
			ctrl1_update |=0x01 ;/*set Enable continuous reading*/
		}

		/*check  continuous update at 1HZ  &  UV data[REG 0x28] MSB & LSB have been read*/  
		if(mode_update_type & 0x04)
		{
			ctrl1_update |=0x00 ;/*continuous update at 1HZ*/	
		}

		if(mode_update_type & 0x08)
		{
			ctrl1_update |=0x02 ;/*update until  UV data[REG 0x28] MSB & LSB have been read !*/
		}	
...
CHAR8S UVIS25_SET_THRESHOLD(CHAR8U thr_data)
{
	...
	CHAR8U data = thr_data;
		...
		if(status!=1)
		{
			return -1; /*write REG_THU_UV  reg fail.*/
		}
...
CHAR8S UVIS25_READ_INTERRUPT_STATUS(CHAR8U *int_status)
{
	...
	CHAR8U data = 0;
		...
		if(status!=1)
		{
			return -1; /*read  REG_INT_SOURCE fail*/
		}
		*int_status = data;
